...
**Generated:** 2026-08-11 (updates the 2026-08-01 version of this same monthly file)
**Hardware target:** Apple M5 Max · 128 GB unified memory · 614 GB/s bandwidth
**Research window:** Last 60 days (2026-06-12 → 2026-08-11)

> **Diffs from previous refresh (`ollama-models-2026-08.md`, generated 2026-08-01, commits `e2a8007`/`20526fc`):**
>
...
>    today — OpenMDW-1.1 license) — NVIDIA's own HF card lists only 52.80% SWE-bench
>    Verified, well below every current code-implementer pick, despite fast claimed
>    throughput ("4× higher throughput vs. similar open models," unverified
>    independently this cycle). Its Ollama `q4_K_M`-labeled tag lists 25 GB for a 30B
>    model — **arithmetic gate flag:** that implies ~6.7 bits/weight, well above
>    typical Q4_K_M (~4.5–5 bpw); plausibly explained by its hybrid Mamba+Transformer
>    architecture keeping Mamba layers at higher precision, but not confirmed this
>    cycle — noting the discrepancy rather than silently repeating the "4-bit" label.
>    Not promoted to any role. See §2.
>
...
All oMLX figures above are dated within or right at the edge of this cycle's
60-day window and are new to this doc.
...
model single-stream. The new oMLX row (7.1–7.2 tok/s) independently confirms this
...
(pre-window, March–April 2026) remains the primary source for older Ollama/Ollama
MLX column figures not updated this cycle; PromptQuorum and Presenc AI for older
mlx_lm figures. The Wale Akinfaderin Medium article ("Benchmarking Open-Weights
LLMs on the MacBook Pro M5 Max") is now fetchable (its earlier HTTP 503 cleared)
...
- **MLX vs llama.cpp:** Ollama's bundled MLX engine gives 20–50% uplift over its
  ...
  `mlx_lm`/MLX-family tools directly (including the new oMLX data source) can add
  further uplift on top. For large dense models near the bandwidth ceiling (e.g.
  Mistral Medium 3.5 at 80 GB), the uplift compresses toward zero — the new oMLX
  measurement (7.1–7.2 tok/s) confirms this is a hard physical limit, not a
  software gap.
- **Ollama v0.32.6–v0.32.9 (new this refresh):** four releases landed since the
  last refresh's v0.32.5 (2026-07-27): **v0.32.6** (Aug 4) ships Qwen3.5
  speculative decoding via its MTP head on Apple GPUs plus an OpenAI-wire-format
  streaming rework; **v0.32.7** (Aug 10) introduces Muse Glimmer support,
  Apple/MLX-only at first; **v0.32.8** (Aug 10, same day) extends Muse Glimmer to
  NVIDIA/AMD; **v0.32.9** (Aug 11, today) adds NVIDIA Nemotron 3.5 Lightning
  support and fixes a Muse Glimmer function-calling parser bug. **Current stable
  is v0.32.9**, not v0.32.5. None of these releases touch Ornith, Llama 4,
  DeepSeek-R1, or KV-cache handling directly.
- **MoE-A3B behaviour:** Models like **Ornith-1.0-35B** (new top code-implementer
  ...
  **Qwen 3.5 30B-A3B** (30B/3B active, ~17 GB) activate only ~3B parameters per
  ...
  pattern in the current Ollama library, and this cycle's fresh oMLX data confirms
  it holds up under direct M5 Max measurement, not just theory.
- **KV-cache headroom:** New this cycle — a directly-measured KV-cache study
  (Contra Collective, MLX 0.21, M5 Max 128GB, pre-window but not previously cited
  in this doc) quantifies the tradeoff concretely for a 70B Q4 model: at 64K
  context, KV cache alone costs 20.8 GB at fp16, 10.4 GB at Q8 (near-lossless,
  ~0.2 MMLU pts), or 5.2 GB at Q4 (8–14pp needle-in-haystack accuracy loss).
  Decode throughput scales inversely: 7.9 → 13.2 (+67%) → 16.1 tok/s (+104%) going
  fp16 → Q8 → Q4 KV cache. At 128 GB total, reserve ~10–18 GB for macOS +
  KV-cache; practical ceiling for a single loaded model remains ~110 GB.
- **M5 Max vs M4 Max:** ~28% higher decode throughput reported across community
  ...
  vs M4 Max comparison found this cycle; figure carried forward unchanged.

### What Exceeds 128 GB (Cloud-Only or Borderline)

| Model | Why it won't fit |
|-------|------------------|
| **Kimi K3** (Moonshot, 2.8T/50.4B active MoE) | Ollama offers only `kimi-k3:cloud` (re-confirmed this cycle); llama.cpp support remains an unmerged PR. Native weights ~1.4 TB; even 2-bit quants ≈ 700+ GB. **Cloud-only.** |
| **DeepSeek V4-Flash** *(new this cycle)* (DeepSeek, 284B/13B active MoE) | Ollama tag confirmed present but exact listed size could not be fetched this cycle. Arithmetic: 284B × ~4.5 bpw ÷ 8 ≈ 160 GB at Q4 — exceeds 128 GB regardless of the precise figure. **Cloud-only by arithmetic; flagging as not independently tag-verified this cycle rather than omitting.** |
| **Ornith-1.0-397B** (deepreinforce-ai, 397B/~3B active MoE) | Unsloth GGUF exists. Q4_K_M ≈ 200 GB (OOM); extreme Q2 ≈ 100 GB fits numerically but quality untested at that quant. **Effectively cloud-only.** Note: the same family's 9B and now 35B members fit comfortably and are this doc's current code-implementer picks — see §3.2. |
| **GLM-5.2** (Z.ai, 744B/40B active MoE) | 62.1% SWE-bench Pro, 91.2% GPQA Diamond; smallest usable GGUF ~217 GB; Ollama only offers `:cloud`. A "GLM-5.5" successor is rumored (analyst projection, not an official Z.ai announcement) — not treated as a real release this cycle. |
| **MiniMax M3** (MiniMax, ~428B/23B active MoE) | 80.5% SWE-bench Verified; smallest local quant ~143 GB; Ollama only offers `:cloud`. (MiniMax's newer "H3" is video/multimodal, not a text LLM, and its weights had not shipped as of the most recent source checked this cycle — not evaluated as a candidate.) |
...
| **DeepSeek-V4-Pro-Max** (1.6T/49B active MoE) | 80.6% SWE-bench Verified — still one of the highest open-weight scores found — but ~800 GB minimum at Q4; a circulating "~50 GB" claim is arithmetically impossible for 1.6T params and was rejected two cycles ago |
| DeepSeek-V3 full (671B MoE, 37B active) | Q4 ≈ 170 GB — borderline, likely OOM with KV-cache |
| **Qwen3.8-Max** *(new this cycle)* (Alibaba, 2.4T total, ~95B active) | Officially launched 2026-08-03; open weights **not yet released** as of this cycle despite a "within about a week" promise reported 2026-08-10. No Ollama tag exists. Not evaluable as a local candidate regardless of size. |
...
|-------|--------|----------|-----------------|
| **Ornith-1.0-35B** *(promoted this cycle)* | deepreinforce-ai (MoE 35B/~3B active) | Jun 25, 2026 (score confirmed this cycle) | **New code-implementer top pick.** 75.6% SWE-bench Verified — now confirmed by primary (DeepReinforce blog + HF card, matching methodology) and independent secondary (BenchLM.ai) — beats prior top pick Laguna XS 2.1 (70.9%) at a comparable 21 GB Q4 footprint and, per fresh oMLX M5 Max data, comparable or better throughput. Sibling of the existing smaller pick, Ornith-1.0-9B. MIT license, 256K context. |
...
| **NVIDIA Nemotron 3.5 Lightning** | NVIDIA (hybrid Mamba+MoE, 30B total/3B active) | Aug 11, 2026 (today) | Successor to Nemotron 3 Nano, built for "always-on agent execution layers"; NVIDIA's own card lists only 52.80% SWE-bench Verified — well below every code-implementer pick in this doc. Ollama's `q4_K_M` tag lists 25 GB for a nominally-4-bit 30B model (~6.7 bpw implied) — arithmetic flag, not fully explained this cycle. **Not promoted to any role.** |
| **DeepSeek V4-Flash** | DeepSeek (MoE 284B/13B active) | Jul 31, 2026 (Ollama listing updated within this window) | New DeepSeek release; ~160 GB at Q4 by arithmetic — **cloud-only, doesn't fit 128 GB.** See §1. |
| **Qwen3.8-Max** | Alibaba/Qwen (MoE 2.4T/~95B active) | Launched Aug 3, 2026 (previewed Jul 19) | Open weights promised "within about a week" as of an Aug 10 update but **not yet shipped**; no Ollama tag. Not locally relevant this cycle. |
| **Laguna XS 2.1** | Poolside (MoE 33B/3B active) | Jul 2, 2026 | **Beaten as code-implementer top pick this cycle by Ornith-1.0-35B** (see above) — remains a fully valid alternative with a longer field track record and its own newly-measured strong M5 Max throughput (87–107 tok/s, oMLX). Not deprecated. |
| **Kimi K3** | Moonshot AI (MoE 2.8T/50.4B active) | Jul 16, 2026 (announced); open weights Jul 26, 2026 | Still no local GGUF/Ollama/MLX support (`kimi-k3:cloud` only), re-confirmed this cycle. **Cloud-only — see §1.** |
| **Mistral Medium 3.5** | Mistral AI (dense 128B) | Apr 29, 2026 | 77.6% SWE-bench Verified; ~80 GB Q4 (confirmed prior cycle); new oMLX data this cycle (7.1–7.2 tok/s) independently confirms the bandwidth-ceiling tok/s estimate is accurate, not just theoretical. Still too slow for agentic loops — quality-critical one-shot use only. |
| **Ollama v0.32.9** | Platform release | Aug 11, 2026 (today) | Current stable, up from v0.32.5. Four releases landed this window — see Runtime Notes for the full v0.32.6–v0.32.9 changelog (Qwen3.5 speculative decoding, Muse Glimmer support, Nemotron 3.5 Lightning support). |

### Notable models evaluated and excluded (unchanged from prior cycles, re-checked)

| Model | Family | Released | Why noted |
|-------|--------|----------|-----------|
| **GLM-5.2** | Z.ai (MoE 744B/40B active) | Jun 13, 2026 | Doesn't fit 128 GB; `:cloud` only. A "GLM-5.5" successor is analyst speculation, not an official release — not treated as new this cycle. |
...
| **Ornith-1.0-35B** | **75.6%** | **Yes (~21 GB Q4) — new top local coding pick** |
| **Laguna XS 2.1** | **70.9%** | **Yes (~20 GB Q4) — beaten as top pick, still a valid alternative** |
| **Ornith-1.0-9B** | **69.4%** | **Yes (~5.6 GB Q4) — smaller coding pick, unchanged** |
...
Source: [vals.ai SWE-bench leaderboard](https://www.vals.ai/benchmarks/swebench) and [BenchLM.ai](https://benchlm.ai/benchmarks/sweVerified) (both primary-adjacent/secondary, cross-checked this cycle); Ornith-1.0-35B and Muse Glimmer scores verified directly against primary vendor sources per §3.2/§3.1.
...
| | Model | Q4 resident | Key benchmarks | Ollama tag |
|-|-------|------------|-----------------|------------|
| **Top pick** | **Ornith-1.0-35B** *(changed this cycle)* | ~21 GB | SWE-bench Verified 75.6%; Terminal-Bench 2.1 64.2%; 256K ctx | `ornith:35b` |
| **Smaller pick** | Ornith-1.0-9B | ~5.6 GB | SWE-bench Verified 69.4%; Terminal-Bench 2.1 43.1; 256K ctx | `ornith:9b` |

**Rationale — top pick changed this cycle:** Ornith-1.0-35B (deepreinforce-ai, MoE 35B total/~3B active, MIT license) is the sibling of the existing smaller pick, Ornith-1.0-9B, sharing the same self-scaffolding agentic-RL training approach. Last cycle this doc explicitly checked for its SWE-bench Verified score and found nothing — it was marked `[unverified]` and not promoted. This cycle the score (75.6%) resolves cleanly against DeepReinforce's own blog and Hugging Face model card (primary, both stating the same methodology: OpenHands harness, temp=1.0, top_p=0.95, 256K context, averaged over 5 runs) and is independently matched by BenchLM.ai (secondary) — passing all three of this doc's verification gates. It beats the prior top pick, Laguna XS 2.1 (70.9%), by 4.7 points at a comparable resident size (21 GB vs ~20 GB) and the same MoE-~3B-active architecture pattern. **On the speed tradeoff specifically** (this doc's selection criterion that throughput is part of quality): fresh oMLX M5 Max data (2026-07-02, batch-size sweep) shows Ornith-1.0-35B running 77–123 tok/s depending on batch size, comparable to or faster than Laguna XS 2.1's own newly-measured 87–107 tok/s (oMLX, 2026-08-10/11) — this is a case where the higher-scoring model does *not* trade away speed, unlike some promotions in this doc's history. Ollama tag `ornith:35b` confirmed at 21 GB directly against the live tags page.

**Smaller pick unchanged:** Ornith-1.0-9B (dense 9B, same family) remains the smaller pick at ~5.6 GB — no challenger beat it this cycle.

**Also evaluated, not selected:**
- *Laguna XS 2.1* (Poolside, Jul 2, 2026; MoE 33B/3B active; 70.9% SWE-bench Verified; `laguna-xs-2.1:q4_K_M`, ~20 GB, Apache-compatible): **beaten as top pick this cycle**, not deprecated. Remains a fully valid alternative — five-plus weeks of field use vs. Ornith-1.0-35B's shorter track record, and its own very competitive newly-measured M5 Max throughput. Teams already standardized on Laguna's tool-calling behavior have no urgent reason to switch.
...
- *Mistral Medium 3.5* (128B dense; 77.6% SWE-bench Verified — higher than either current pick; `mistral-medium-3.5:128b`, 80 GB Q4): fits, but at ≈7.1–7.2 tok/s (now confirmed by direct oMLX measurement, not just bandwidth arithmetic) a 1K-token completion takes roughly 140–150 seconds versus Ornith-1.0-35B's ~8–13 seconds at the low end of its measured range — the latency penalty rules it out for agentic loops. Quality-critical one-shot tasks only.
- *Ornith-1.0-397B* (82.4%), *DeepSeek-V4-Pro-Max* (80.6%), *MiniMax M3* (80.5%), *Kimi K3* (93.4% per vals.ai) top the global leaderboard but none fit 128 GB at practical quants — see §1.
...
**Rationale:** DeepSeek-R1-Distill-Qwen-32B provides always-on chain-of-thought reasoning and holds the highest MATH benchmark score of any locally-runnable model under 40 GB. Distilled from the 671B R1 teacher; generates full reasoning traces before answering. New this cycle: fresh oMLX M5 Max data (2026-07-15, 4bit) directly measures 27.6–28.9 tok/s at short context — consistent with this doc's existing Ollama-MLX estimate. Qwen3.6-27B with `/think` is the pragmatic alternative if you want to avoid loading a second model. No challenger found this cycle.
...
**Rationale:** For judging, **disable thinking mode** — reasoning traces inflate token cost and can introduce self-consistency bias in scores. Qwen3.6-27B's instruction fidelity and calibration make it the recommended local judge. Inject a scored reference example at the top of the judge prompt to anchor the scoring scale. **Staleness flag, unchanged this cycle:** no fresher RewardBench 2 data was found — last confirmed data remains the Jun 2025 submission behind RewardBench 2's ICLR 2026 paper. For high-stakes judging, frontier API models remain preferred; use local judges for cost-sensitive or privacy-constrained pipelines.
...
**Rationale:** Gemma 4's 26B MoE variant (Google, Apr 2026) understands images, text, tables, and LaTeX natively in a single forward pass. It handles complex PDF layouts, nested tables, and mixed-language documents reliably. No update found in this window. Mistral OCR 4 remains a real, purpose-tuned upgrade for dedicated OCR pipelines run *outside* Ollama (still no evidence of a GGUF/Ollama-loadable form); within the Ollama-only constraint of this document, Gemma 4 12B Unified (§3.1) remains the fallback for lighter-weight document tasks.
...
### 4.1 MoE-A3B Models
**Ornith-1.0-35B** (new top code-implementer pick, 35B/~3B active, ~21 GB at Q4_K_M), **Laguna XS 2.1** (33B/3B active, ~20 GB), and **Qwen 3.5 30B-A3B** (30B/3B active, ~17 GB) all activate only ~3B parameters per token despite loading a much larger weight set. This cycle's fresh oMLX M5 Max data confirms the pattern holds up under direct measurement: 77–123 tok/s for Ornith-1.0-35B, 87–107 tok/s for Laguna XS 2.1 — both firmly in "interactive agentic loop" territory despite 30B+ class quality. This remains the highest quality-per-GB pattern in the current Ollama library.
...
**Ornith-1.0-9B** is a *dense* 9B model, trained specifically around self-generated agentic-RL scaffolds, that beats larger competitors at a fraction of their resident memory. Its 35B sibling promoted to top pick this cycle (§3.2) suggests the self-scaffolding training approach scales cleanly across sizes within the same family, not just as a one-off small-model trick.
...
| **Ornith-1.0-35B** *(new top pick)* | `ornith:35b` | ~21 GB | Code implementer (top pick, higher SWE-bench) |
...
| *Laguna XS 2.1 (optional, now alternative)* | `laguna-xs-2.1:q4_K_M` | ~20 GB | *Code implementer alt — longer field track record, very competitive speed* |
...
| **Ornith-1.0-35B + Ornith-1.0-9B** *(new)* | ~26.6 GB | Same-family coding pair: escalate from smaller to top pick within one session without a family-behavior mismatch |
...
| **Scout + Ornith-1.0-35B** *(new)* | ~88 GB | Orchestrated multi-file coding with the new top code pick |
...
| DeepSeek V4-Flash (284B total) *(new)* | ~160 GB at Q4 by arithmetic |
...
ollama pull ornith:35b                # ~21 GB  — code implementation (TOP pick, NEW this cycle)
...
# ollama pull laguna-xs-2.1:q4_K_M      # ~20 GB  — code implementation alt (beaten as top pick this cycle, still valid — longer track record)
# ollama pull devstral-small-2          # ~15 GB  — code implementation alt (battle-tested tool-calling)
# ollama pull mistral-medium-3.5:128b   # ~80 GB  — 77.6% SWE-bench but ~7 tok/s (confirmed via direct M5 Max measurement this cycle)
...
# Agentic coding, TOP pick (Ornith-1.0-35B) — NEW this cycle
...
- [Ollama Releases — GitHub](https://github.com/ollama/ollama/releases) (primary — v0.32.6/.7/.8/.9 confirmed this cycle)
...
- [kimi-k3](https://ollama.com/library/kimi-k3) — re-confirmed cloud-only tag

### Apple Silicon Benchmarks
- [oMLX Community M5 Max Benchmarks](https://omlx.ai/benchmarks) (new primary/crowdsourced source this cycle — individually-dated, directly-measured M5 Max figures for Ornith-1.0-9B/35B, Laguna XS 2.1, DeepSeek-R1-Distill-32B, Llama 3.1 8B, Llama 3.3 70B, Mistral Medium 3.5)
...
- *Wale Akinfaderin, "Benchmarking Open-Weights LLMs on the MacBook Pro M5 Max" (Medium) — HTTP 503 cleared this cycle but content is paywalled beyond the intro; confirmed intro benchmarks the binned 32-core/460 GB/s M5 Max SKU, not this doc's 40-core/614 GB/s target — low value even if fully accessible.*

### Ornith-1.0 (top pick promoted this cycle)
...
- [deepreinforce-ai/Ornith-1.0-9B — Hugging Face](https://huggingface.co/deepreinforce-ai/Ornith-1.0-9B) (primary, smaller pick, unchanged)

### Muse Glimmer (new, evaluated not promoted)
...
### NVIDIA Nemotron 3.5 Lightning (new, evaluated not promoted)
...
- Qwen3.8-Max — secondary coverage only (yottalabs.ai); open weights not yet released, no Ollama tag, not locally relevant

### Laguna XS 2.1 (beaten as top pick, remains alternative)
...
### Kimi K3 / Moonshot
- [Kimi K3 — Ollama](https://ollama.com/library/kimi-k3) (primary — cloud-only tag re-confirmed)
...
### GLM-5.2 / Z.ai, MiniMax M3, Mistral, Devstral, Cohere (carried forward, unchanged)
